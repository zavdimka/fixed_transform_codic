module hevc_luma_ctu64_idr_nal #(
    ...
    wire last_tu_done = bridge_done && ctu_active &&
        (cu_index == 4'd15);
    wire joined_ctu_done = ctu_active &&
        (last_tu_done_seen || last_tu_done) &&
    ...
    assign current_cu_index = cu_index;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (bridge_done && ctu_active) begin
                if (cu_index != 4'd15) begin
                    cu_index <= cu_index + 1'b1;
                end else begin
                    last_tu_done_seen <= 1'b1;
                end
            end
