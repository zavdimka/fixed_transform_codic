module hevc_dual_reconstruction_core (
    ...
    logic bank_size8 [0:3], bank_error [0:3];
    logic [1:0] write_bank, read_bank, forward_bank, inverse_bank;
    logic f_size8, f_chroma, i_size8;
    logic [1:0] f_quality;
    logic [7:0] input_address, replay_address;
    ...
    logic i_m_last;
    logic f_done, f_error, f_busy, i_done, i_error, i_busy;
    ...
    wire [7:0] f_final = f_size8 ? 8'd63 : 8'd255;
    wire [7:0] i_final = i_size8 ? 8'd63 : 8'd255;
    ...
    function automatic logic [7:0] raster_address(
        input logic size8, input logic [3:0] x, input logic [3:0] y);
    ...
    forward_transform (
        ...
        .m_x(f_m_x), .m_y(f_m_y), .m_block_last(f_m_last),
        .done(f_done), .protocol_error(f_error), .busy(f_busy)
    );
    assign f_m_ready = (fstate == F_QUANT) && quant_s_ready;
    ...
    hevc_shared_quant_dequant quant (
        .clk, .rst_n, .s_valid((fstate == F_QUANT) && f_m_valid),
    ...
    assign coefficient_valid = (fstate == F_QUANT) && quant_m_valid;
    assign quant_m_ready = (fstate == F_QUANT) && coefficient_ready;
    ...
    hevc_prediction_buffer16 pred_bank0 (
        ...
        .read_address(raster_address(i_size8, i_m_x, i_m_y)),
        .read_data(pred0_rd));
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            f_size8 <= 1'b0; f_chroma <= 1'b0; f_quality <= 2'd1;
            i_size8 <= 1'b0; input_address <= '0; replay_address <= '0;
            ...
            if (command_fire) begin
                ...
                f_size8 <= command_size8; f_chroma <= command_chroma;
                f_quality <= command_quality; input_address <= '0;
            ...
            if (replay_fire && replay_issue_done) begin
                residual_valid <= 1'b0; istate <= I_RECON;
            end
            if ((istate == I_RECON) && prediction_ready) begin
    ...
    logic unused_status;
    assign unused_status = ^{f_done, f_error, f_busy, i_done, i_error,
        i_busy, chroma_qp};
endmodule
