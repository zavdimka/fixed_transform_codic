module hevc_chroma_tu8_cabac_bridge (
    input  logic               clk,
    input  logic               rst_n,
    ...
    logic descriptor_cbf_register;

    logic replay_active;
    logic [5:0] replay_read_address;
    logic replay_output_valid;
    logic [5:0] replay_output_address;
    ...
    logic [5:0] loop_coefficient_write_address;
    ...
    assign descriptor_valid = descriptor_pending;
    assign descriptor_cbf = descriptor_cbf_register;
    ...
    hevc_chroma_tu8_reconstruction_loop reconstruction_loop (
        .clk(clk),
        .rst_n(rst_n),
    ...
    hevc_coefficient_buffer8 coefficient_buffer (
        .clk(clk),
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            descriptor_cbf_register <= 1'b0;
            replay_active <= 1'b0;
            replay_read_address <= 6'd0;
            replay_output_valid <= 1'b0;
            replay_output_address <= 6'd0;
            ...
            if (loop_coefficient_write_enable) begin
                ...
                if (loop_coefficient_block_last) begin
                    ...
                    descriptor_cbf_register <= coefficient_block_nonzero;
            ...
            if (replay_can_read) begin
                ...
                replay_output_last <= replay_read_address == 6'h3f;
                if (replay_read_address == 6'h3f) begin
                    replay_active <= 1'b0;
                end else begin
