module hevc_ctu64_syntax_scheduler (
    input  logic       clk,
    input  logic       rst_n,
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (coefficient_block_done && (state != COEFFICIENT)) begin
                protocol_error <= 1'b1;
            end
            if (coefficient_valid && coefficient_ready &&
                !coefficient_kind_valid) begin
                protocol_error <= 1'b1;
            end
            ...
            case (state)
                ...
                PREFIX: begin
                    if (prefix_done) begin
                        if (cu_luma_cbf_register) begin
                            state <= COEFFICIENT;
                        end else if (cu_index == 4'd15) begin
                            state <= TERMINATE;
                        end else begin
