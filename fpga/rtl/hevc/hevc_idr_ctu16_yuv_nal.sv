module hevc_idr_ctu16_yuv_nal #(
    ...
    localparam logic [7:0] CTU_COLUMNS_VALUE = 8'(CTU_COLUMNS);
    ...
    localparam logic [6:0] SLICE_CTU_ROWS_VALUE = 7'(SLICE_CTU_ROWS);
    ...
    logic [6:0] ctu_x_register;
    logic [5:0] ctu_row_in_slice;
    ...
    logic cabac_y_ready;
    logic cabac_cb_ready;
    logic cabac_cr_ready;
    logic cabac_m_valid;
    logic cabac_m_ready;
    logic [7:0] cabac_m_byte;
    logic cabac_m_last;
    logic cabac_y_block_done;
    logic cabac_cb_block_done;
    logic cabac_cr_block_done;
    ...
    wire parameters_valid = ({1'b0, slice_row} < SLICE_COUNT_VALUE) &&
        (qp <= 51) && (CTU_COLUMNS > 0) && (CTU_COLUMNS <= 128) &&
        (CTU_ROWS > 0) && (CTU_ROWS <= 64) &&
        (SLICE_CTU_ROWS > 0) && (SLICE_CTU_ROWS <= CTU_ROWS) &&
        ((CTU_ROWS % SLICE_CTU_ROWS) == 0);
    ...
    assign y_ready = (state == ACTIVE) && cabac_y_ready;
    assign cb_ready = (state == ACTIVE) && cabac_cb_ready;
    assign cr_ready = (state == ACTIVE) && cabac_cr_ready;
    assign cabac_m_ready = nal_s_ready;

    assign current_ctu_x = ctu_x_register;
    /* verilator lint_off WIDTHEXPAND */
    /* verilator lint_off WIDTHTRUNC */
    assign current_ctu_y = slice_row_register * SLICE_CTU_ROWS +
        ctu_row_in_slice;
    /* verilator lint_on WIDTHTRUNC */
    /* verilator lint_on WIDTHEXPAND */
    assign y_block_done = cabac_y_block_done;
    assign cb_block_done = cabac_cb_block_done;
    assign cr_block_done = cabac_cr_block_done;
    assign ctu_done = cabac_ctu_done;
    assign parameter_error = wrapper_parameter_error || context_init_error ||
        nal_parameter_error;
    assign protocol_error = wrapper_protocol_error || cabac_protocol_error;
    assign busy = (state != IDLE) || cabac_busy || nal_busy;

    hevc_ctu16_yuv_cabac cabac_path (
        .clk(clk), .rst_n(rst_n),
        .cfg_valid(1'b0), .cfg_ready(unused_cfg_ready),
        .cfg_context_address(8'd0), .cfg_state_index(6'd0), .cfg_mps(1'b0),
        .context_init_valid(context_init_valid),
        .context_init_ready(context_init_ready),
        .context_init_slice_type(2'd2), .context_init_qp(qp_register),
        ...
        .ctu_last_in_slice({1'b0, ctu_x_register} ==
            CTU_COLUMNS_VALUE - 1'b1 &&
            {1'b0, ctu_row_in_slice} == SLICE_CTU_ROWS_VALUE - 1'b1),
        .cu_valid(cu_valid && (state == ACTIVE)), .cu_ready(cabac_cu_ready),
        .cu_luma_mode_dc(cu_luma_mode_dc), .cu_luma_cbf(cu_luma_cbf),
        .cu_cb_cbf(cu_cb_cbf), .cu_cr_cbf(cu_cr_cbf),
        .y_valid(y_valid && (state == ACTIVE)), .y_ready(cabac_y_ready),
        .y_raster_address(y_raster_address), .y_coefficient(y_coefficient),
        .y_block_last(y_block_last),
        .cb_valid(cb_valid && (state == ACTIVE)), .cb_ready(cabac_cb_ready),
        .cb_raster_address(cb_raster_address),
        .cb_coefficient(cb_coefficient), .cb_block_last(cb_block_last),
        .cr_valid(cr_valid && (state == ACTIVE)), .cr_ready(cabac_cr_ready),
        .cr_raster_address(cr_raster_address),
        .cr_coefficient(cr_coefficient), .cr_block_last(cr_block_last),
        .m_valid(cabac_m_valid), .m_ready(cabac_m_ready),
        .m_byte(cabac_m_byte), .m_last(cabac_m_last),
        .y_block_done(cabac_y_block_done),
        .cb_block_done(cabac_cb_block_done),
        .cr_block_done(cabac_cr_block_done), .ctu_done(cabac_ctu_done),
        .slice_done(cabac_slice_done),
        .protocol_error(cabac_protocol_error), .busy(cabac_busy)
    );
    ...
    hevc_idr_slice_nal #(
        ...
        .clk(clk), .rst_n(rst_n), .start_valid(nal_start_valid),
        .start_ready(nal_start_ready), .slice_row(slice_row_register),
        .qp(qp_register), .no_output_of_prior_pics(no_output_register),
        .s_valid(cabac_m_valid), .s_ready(nal_s_ready),
        .s_byte(cabac_m_byte), .s_last(cabac_m_last),
        .m_valid(m_valid), .m_ready(m_ready), .m_byte(m_byte), .m_last(m_last),
        .done(nal_done), .busy(nal_busy),
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                ACTIVE: begin
                    if (ctu_start_fire &&
                        ({1'b0, ctu_x_register} >= CTU_COLUMNS_VALUE))
                        wrapper_protocol_error <= 1'b1;
                    /* verilator lint_off UNSIGNED */
                    if (cabac_ctu_done) begin
                        if ({1'b0, ctu_x_register} <
                            CTU_COLUMNS_VALUE - 1'b1) begin
                            ctu_x_register <= ctu_x_register + 1'b1;
                        end else if ({1'b0, ctu_row_in_slice} <
                            SLICE_CTU_ROWS_VALUE - 1'b1) begin
                            ctu_x_register <= 7'd0;
                            ctu_row_in_slice <= ctu_row_in_slice + 1'b1;
                        end
                    end
